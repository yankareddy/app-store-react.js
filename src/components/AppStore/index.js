...
class AppStore extends Component {
  ...
  getFilteredTabList = () => {
    ...
    const filteredTabList = appsList.filter(
  ...
  render() {
    ...
    const filteredTabList = this.getFilteredTabList()
    const searchResult = filteredTabList.filter(eachApp =>
    ...
    return (
      <div className="main-container">
        ...
        <ul className="app-item-container">
          {searchResult.map(eachmap => (
            <AppItem eachmap={eachmap} key={eachmap.appId} />
          ))}
        </ul>
      </div>
    )
